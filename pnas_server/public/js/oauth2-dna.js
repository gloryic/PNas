DNA.OAuth2 = {
	init: function() {
		...
		DNA.socket.on('toclient',function(data){
			 console.log(data.msg);
			 DNA.OAuth2.viewDialog(data.msg,"left");
		});
	},
		...
		$('.kakaoInput').bind('keydown', function(e) {
			...
			if(code == 13){
			 		...
			 		if(message!=''&&message!='/clean'){
			 			DNA.socket.emit('fromclient',{msg:message});
					}
    ...
    viewDialog : function(message,direction){
    	...
    	if(!direction.localeCompare('right'))
    	{
	    	$(".cont_help .inner_cont").append($("<div class='talk_tool clearfix'>"
	    							  +"<div class='wrap_img'>"
	    								+"<img width='42' height='50' alt='' src="+window.location.origin+"/images/img_men.png>"
